...
const FeatureItem = ({
  ...
  <div className="flex items-center gap-3 py-2 group">
    ...
    <span className={`transition-colors duration-200 ${included
...
const PriceCard = ({
  ...
  return (
    <div className="relative">
      <GlassmorphicCard
        className={`p-8 flex flex-col h-full hover-lift group transition-all duration-500 ${popular ? 'scale-105 border-blue-500/30 shadow-2xl shadow-blue-500/20' : ''
          }`}
        variant={popular ? "frosted" : "gradient"}
        glowEffect={popular}
        hoverEffect
      >
        {/* Badges - Popular and Save badges side by side at top */}
        <div className="absolute -top-6 right-2 z-10 flex items-center gap-2">
          {popular && (
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white text-xs font-medium py-1 px-3 rounded-full shadow-lg flex items-center gap-1">
              <Star className="w-3 h-3 fill-current" />
              Popular
            </div>
          )}
          {badge && (
            <div className="bg-gradient-to-r from-yellow-400 to-orange-400 text-black text-xs font-bold py-1 px-3 rounded-full shadow-lg">
              {badge}
            </div>
          )}
        </div>
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${popular
